...
import {
  Transfer,
  TransferBatch,
  CreateTransferTedInput,
  CreateTransferBatchTedInput,
  ListTransfersFilters,
  ApiResponse,
} from '../types/api.js';

const BASE_PATH = '/v2/transfer/ted';
const BATCH_PATH = '/v2/transfer/ted_batches';

export async function listTransferTed(
...
export async function createTransferTed(
  client: KobanaApiClient,
  input: CreateTransferTedInput,
...
export async function createTransferTedBatch(
  client: KobanaApiClient,
  input: CreateTransferBatchTedInput,
  idempotencyKey?: string
