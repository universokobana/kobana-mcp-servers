...
function handleInfo(_req: IncomingMessage, res: ServerResponse): void {
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify({
    name: 'kobana-mcp-data',
    version: '1.0.0',
    description: 'MCP Server for Kobana Data API v2 (Bank Billet Queries)',
    endpoints: {
      sse: '/sse',
      messages: '/messages',
      health: '/health',
...
httpServer.listen(PORT, HOST, () => {
  console.log(`Kobana MCP Data Server (HTTP) running on http://${HOST}:${PORT}`);
