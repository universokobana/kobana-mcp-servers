...
function handleInfo(_req: IncomingMessage, res: ServerResponse): void {
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify({
    name: 'kobana-mcp-admin',
    version: '1.0.0',
    description: 'MCP Server for Kobana Admin API v2 (users, subaccounts, connections, certificates)',
    endpoints: {
      sse: '/sse',
      messages: '/messages',
      health: '/health',
    ...
    tools: [
      ...
      'update_admin_user',
      'delete_admin_user',
      'list_admin_subaccounts',
      'create_admin_subaccount',
      'get_admin_subaccount',
      'update_admin_subaccount',
      'list_admin_certificates',
      'create_admin_certificate',
      'list_admin_connections',
      'create_admin_connection',
      'get_admin_connection',
      'update_admin_connection',
      'delete_admin_connection',
      'create_admin_connection_association',
      'delete_admin_connection_association',
    ],
  }));
}
...
httpServer.listen(PORT, HOST, () => {
  console.log(`Kobana MCP Admin Server (HTTP) running on http://${HOST}:${PORT}`);
